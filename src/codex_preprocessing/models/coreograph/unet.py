...
from .ftools import loadData, pathjoin, saveData
from .imtools import im2double, imwrite, normalize, tifread
from .partition_image import PI2D
    ...
    def setup(
        ...
        def down_samp_layer(data, index):
            with tf.name_scope("ld%d" % index):
                ...
                ldXWeightsShortcut = tf.Variable(
                    tf.truncated_normal([1, 1, nOutX[index], nOutX[index + 1]], stddev=stdDev0),
        ...
        def up_samp_layer(data, index):
            with tf.name_scope("lu%d" % index):
                ...
                for i in range(nExtraConvs):
                    cv = tf.nn.relu(
                        tf.nn.conv2d(cv, luXWeightsExtra[i], strides=[1, 1, 1, 1], padding="SAME"),
                        name="conv2Extra%d" % i,
                    )
                return cv
        ...
        for i in range(nTest):
            ...
            if j == batchSize - 1 or i == nTest - 1:
                ...
                for k in range(j + 1):
                    ...
                    imwrite(
                        np.uint8(255 * np.concatenate((im, np.concatenate((pm, gt), axis=1)), axis=1)),
...
def coreSegmenterOutput(I, initialmask, findCenter):
    ...
    if findCenter == True:
        ...
        TMAmask = binary_closing(TMAlabel == index, disk(3))
...
#     print("WARNING! IF USING FOR TISSUE SPLITTING, IT IS ADVISED TO SET --downsampleFactor TO HIGHER THAN DEFAULT OF 5")
...
#         np.savetxt(
#             outputPath + os.path.sep + "centroidsY-X.txt",
...
#         tifffile.imwrite(
#             outputPath + os.path.sep + "Coremask.tif",
#             np.uint8(coreLabel),
#             compression="zlib",
#             predictor=True,
#         )
...
#         initialmask = core[
#             np.uint32(y[iCore] * dsFactor) : np.uint32(yLim[iCore] * dsFactor),
...
#                 coreSlice,
#                 (
#                     int((float(coreSlice.shape[1]) * 0.25)),
#                     int((float(coreSlice.shape[0]) * 0.25)),
#                 ),
#             )
...
#         masksub = resize_mask(
#             resize_mask(TMAmask, (hsize, vsize)),
#             (
#                 int((float(coreSlice.shape[1]) * dsFactor)),
#                 int((float(coreSlice.shape[0]) * dsFactor)),
#             ),
#         )
#         singleMaskTMA[
#             int(y[iCore] * dsFactor) : int(y[iCore] * dsFactor) + masksub.shape[0],
#             int(x[iCore] * dsFactor) : int(x[iCore] * dsFactor) + masksub.shape[1],
#         ] = masksub
...
#             imagesub,
#             str(iCore + 1),
#             (int(P[iCore].centroid[1]), int(P[iCore].centroid[0])),
#             0,
#             0.5,
#             (0, 255, 0),
#             1,
...
#         tifffile.imwrite(
#             maskOutputPath + os.path.sep + str(iCore + 1) + "_mask.tif",
#             np.uint8(TMAmask),
#             compression="zlib",
#             predictor=True,
#         )
...
#     tifffile.imwrite(
#         outputPath + os.path.sep + "TMA_MAP.tif",
#         imagesub,
#         compression="zlib",
#         predictor=True,
#     )
