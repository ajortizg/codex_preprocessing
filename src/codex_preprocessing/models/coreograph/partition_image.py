        ...
        if len(PI2D.Output.shape) == 3:
            if PI2D.Mode == "accumulate":
                for i in range(PI2D.Output.shape[0]):
                    PI2D.Output[i, margin : margin + nr, margin : margin + nc] = np.divide(
                        PI2D.Output[i, margin : margin + nr, margin : margin + nc],
                        C,
                    )
            return PI2D.Output[:, margin : margin + nr, margin : margin + nc]
...
class PI3D:
    ...
    def getValidOutput():
        ...
        if len(PI3D.Output.shape) == 3:
            if PI3D.Mode == "accumulate":
                return np.divide(
                    PI3D.Output[margin : margin + nz, margin : margin + nr, margin : margin + nc],
                    C,
                )
