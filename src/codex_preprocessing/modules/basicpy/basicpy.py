...
from codex_preprocessing.modules.basicpy._jax_routines import ApproximateFit, LadmapFit
from codex_preprocessing.modules.basicpy.metrics import autotune_cost
from codex_preprocessing.modules.basicpy.tools.dct_tools import JaxDCT
from codex_preprocessing.modules.basicpy.types import ArrayLike, PathLike
...
class BaSiC(BaseModel):
    ...
    fitting_mode: FittingMode = Field(FittingMode.ladmap, description="Must be one of ['ladmap', 'approximate']")
    epsilon: float = Field(
        0.1,
    ...
    smoothness_flatfield: float = Field(1.0, description="Weight of the flatfield term in the Lagrangian.")
    smoothness_darkfield: float = Field(1.0, description="Weight of the darkfield term in the Lagrangian.")
    sparse_cost_darkfield: float = Field(0.01, description="Weight of the darkfield sparse term in the Lagrangian.")
    ...
    max_mu_coef: float = Field(1e7, description="Maximum allowed value of mu, divided by the initial value.")
    ...
    def __call__(self, images: np.ndarray, timelapse: bool = False) -> Union[Tuple[np.ndarray, np.ndarray], np.ndarray]:
    ...
    def _resize(self, Im, target_shape):
        if self.resize_mode == ResizeMode.jax:
            ...
            Im = skimage_resize(np.array(Im), target_shape, preserve_range=True, **self.resize_params)
        ...
        if self.working_size is not None:
            if np.isscalar(self.working_size):
                ...
                if not Im.ndim - 2 == len(self.working_size):
                    raise ValueError("working_size must be a scalar or match the image dimensions")
                else:
    ...
    def _perform_segmentation(self, Im):
        ...
        if not self.autosegment:
            ...
            mask = Im < th
            return np.array([binary_erosion(m, ball(self.autosegment_margin)) for m in mask])
        else:
    ...
    def fit(
        ...
        if images.ndim == 3:
            ...
            if self.fitting_mode == FittingMode.approximate:
                raise ValueError("Only 2-dimensional images are accepted for the approximate mode.")
        else:
            raise ValueError("Images must be 3 or 4-dimensional array, " + "with dimension of (T,Y,X) or (T,Z,Y,X).")
        ...
        if self.fitting_mode == FittingMode.approximate:
            ...
            self._smoothness_flatfield = jnp.sum(jnp.abs(mean_image_dct)) / 800 * self.smoothness_flatfield
            self._smoothness_darkfield = self._smoothness_flatfield * self.smoothness_darkfield / 2.5
        ...
        for i in range(self.max_reweight_iterations):
            ...
            if S.max() == 0:
                logger.error("Estimated flatfield is zero. " + "Please try to decrease smoothness_darkfield.")
                raise RuntimeError("Estimated flatfield is zero. " + "Please try to decrease smoothness_darkfield.")
            self._S = S
            self._D_R = D_R
            self._B = B
            self._D_Z = D_Z
            ...
            if last_S is not None:
                ...
                logger.info(f"Iteration {i} elapsed time: " + f"{time.monotonic() - start_time} seconds")
        ...
        self.baseline = B
        logger.info(f"=== BaSiC fit finished in {time.monotonic()-start_time} seconds ===")
    ...
    def transform(
        ...
        if timelapse:
            ...
            if timelapse == TimelapseTransformMode.multiplicative:
                output = (im_float - self.darkfield[np.newaxis]) / self.flatfield[np.newaxis] - self.baseline[baseline_inds]
            elif timelapse == TimelapseTransformMode.additive:
                baseline_flatfield = self.flatfield[np.newaxis] * self.baseline[baseline_inds]
                output = im_float - self.darkfield[np.newaxis] - baseline_flatfield
            else:
                raise ValueError("timelapse value must be bool, 'multiplicative' or 'additive'")
        else:
            output = (im_float - self.darkfield[np.newaxis]) / self.flatfield[np.newaxis]
        logger.info(f"=== BaSiC transform finished in {time.monotonic()-start_time} seconds ===")
        return output
    ...
    def fit_transform(
        ...
        """
        self.fit(images, fitting_weight=fitting_weight, skip_shape_warning=skip_shape_warning)
    ...
    def autotune(
        ...
        *,
        ...
        val_range = (vmax - vmin * vmin_factor) * vrange_factor  # fix the value range for histogram
