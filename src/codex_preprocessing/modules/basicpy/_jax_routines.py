...
class BaseFit(BaseModel):
    ...
    def fit(
        ...
        if W_D.shape != Im.shape[1:]:
            raise ValueError("darkfield weight must have the same shape as images.shape[1:]")
...
class LadmapFit(BaseFit):
    ...
    def _step(
        ...
        S_new = S + jnp.sum(B[:, newax, newax, newax] * (Im - I_B - I_R + Y / mu), axis=0) / eta_S
        S_new = idct3d(_jshrinkage(dct3d(S_new), self.smoothness_flatfield / (eta_S * mu)))
        ...
        if self.get_darkfield:
            ...
            D_R_new = D_R + 1.0 / eta_D * jnp.sum(Im - BS - D_R[newax, ...] - D_Z - I_R + Y / mu, axis=0)
            D_R_new = idct3d(_jshrinkage(dct3d(D_R_new), self.smoothness_darkfield / eta_D / mu))
            D_R_new = _jshrinkage(D_R_new, self.sparse_cost_darkfield * dark_weight / eta_D / mu)
            dD_R = D_R_new - D_R
            D_R = D_R_new
    ...
    def calc_weights(self, I_B, I_R):
        Ws = jnp.ones_like(I_R, dtype=jnp.float32) / (jnp.abs(I_R / (I_B + self.epsilon)) + self.epsilon)
...
class ApproximateFit(BaseFit):
    ...
    def _step(
        ...
        if self.get_darkfield:
            ...
            A = (
                jnp.sum(R * S_inmask[newax, ...], axis=(1, 2)) / jnp.sum(S_inmask * R.shape[0])
                - jnp.sum(R * S_outmask[newax, ...], axis=(1, 2)) / jnp.sum(S_outmask * R.shape[0])
            ) / jnp.mean(R)
            ...
            D_R = (R * B_valid[:, newax, newax]).sum(axis=0) / B_valid.sum() - (B * B_valid).sum() / B_valid.sum() * S
