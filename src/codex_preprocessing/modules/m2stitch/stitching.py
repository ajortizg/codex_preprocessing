...
from ._global_optimization import compute_final_position, compute_maximum_spanning_tree
from ._stage_model import (
...
from ._translation_computation import interpret_translation, multi_peak_max, pcm
from ._typing_utils import BoolArray, Float, NumArray
...
def process_image_pair(
    i2, g, direction, images, position_initial_guess, overlap_diff_threshold, sizeY, sizeX, use_gpu=True
):
    ...
    if position_initial_guess is not None:
        ...
        lims = np.array([get_lims(dimension, size) for dimension, size in zip("yx", [sizeY, sizeX])])
    else:
...
def stitch_images(
    ...
    if position_indices is None:
        if row_col_transpose:
            warnings.warn("row_col_transpose is True. The default value will be changed to False in the major release.")
    ...
    grid["top"] = grid.apply(lambda g: get_index(g["col"], g["row"] - 1), axis=1).astype(pd.Int32Dtype())
    grid["left"] = grid.apply(lambda g: get_index(g["col"] - 1, g["row"]), axis=1).astype(pd.Int32Dtype())
