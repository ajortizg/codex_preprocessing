...
from ._typing_utils import Float, FloatArray, Int, IntArray, NumArray
...
def interpret_translation(
    ...
    _poss = []
    for ymags, xmags, ysign, xsign in itertools.product(ymagss, xmagss, [-1, +1], [-1, +1]):
        yvals = ymags * ysign
        xvals = xmags * xsign
        _poss.append([yvals, xvals])
    poss = np.array(_poss)
    valid_ind = (ymin <= poss[:, 0, :]) & (poss[:, 0, :] <= ymax) & (xmin <= poss[:, 1, :]) & (poss[:, 1, :] <= xmax)
