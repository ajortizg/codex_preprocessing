...
def filter_by_overlap_and_correlation(
    ...
    if overlap_percentage is not None:
        # Use known overlap percentage
        r = (size * (100 - overlap_percentage - pou) / 100, size * (100 - overlap_percentage + pou) / 100)
    else:
        # Use estimated overlap
...
def filter_by_repeatability(grid: pd.DataFrame, r: Float, ncc_threshold: Float) -> pd.DataFrame:
    ...
    for _, grp in grid.groupby("col"):
        ...
        if not any(isvalid):
            # grid["left_valid3"] = grid["left_valid3"].astype(bool)
    ...
    for _, grp in grid.groupby("row"):
        ...
        if not any(isvalid):
            # grid["top_valid3"] = grid["top_valid3"].astype(bool)
...
def replace_invalid_translations(grid: pd.DataFrame) -> pd.DataFrame:
    ...
    for direction in ["left", "top"]:
        for key in ["x", "y", "ncc"]:
            ...
            grid.loc[isvalid, f"{direction}_{key}_second"] = grid.loc[isvalid, f"{direction}_{key}_first"]
    for direction, rowcol in zip(["left", "top"], ["col", "row"]):
        for _, grp in grid.groupby(rowcol):
            isvalid = grp[f"{direction}_valid3"].astype(bool)
            if any(isvalid):
                assert all(pd.isna(grid.loc[grp.index[~isvalid], f"{direction}_y_second"]))
                assert all(pd.isna(grid.loc[grp.index[~isvalid], f"{direction}_x_second"]))
                grid.loc[grp.index[~isvalid], f"{direction}_y_second"] = grp[isvalid][f"{direction}_y_first"].median()
                grid.loc[grp.index[~isvalid], f"{direction}_x_second"] = grp[isvalid][f"{direction}_x_first"].median()
